...
def test_paper_model_valid():
    ...
    paper = Paper(
        paper_id="2401.12345", title="Attention Is All You Need",
        abstract="We propose a new architecture...", authors=["Vaswani", "Shazeer"],
        venue="NeurIPS", date="2017-06-12", arxiv_id="1706.03762",
        citations=100000, quality_score=0.95, extraction_status="pending",
        embedding=[0.1] * 768,
    )
...
def test_paper_model_nullable_venue():
    from lens.store.models import Paper
    paper = Paper(
        paper_id="2401.99999", title="Some Preprint", abstract="Abstract text",
        authors=["Author"], venue=None, date="2024-01-01", arxiv_id="2401.99999",
        citations=0, quality_score=0.1, extraction_status="pending",
        embedding=[0.0] * 768,
    )
...
def test_tradeoff_extraction_model():
    ...
    ext = TradeoffExtraction(
        paper_id="2401.12345", improves="mathematical reasoning accuracy",
        worsens="inference time per token", technique="chain-of-thought with self-consistency",
        context="on GSM8K benchmark", confidence=0.85,
...
def test_architecture_extraction_model():
    ...
    ext = ArchitectureExtraction(
        paper_id="2401.12345", component_slot="attention mechanism",
        variant_name="grouped-query attention", replaces="multi-head attention",
        key_properties="reduces KV cache by sharing keys/values", confidence=0.9,
    )
...
def test_architecture_extraction_replaces_nullable():
    from lens.store.models import ArchitectureExtraction
    ext = ArchitectureExtraction(
        paper_id="2401.12345", component_slot="activation function",
        variant_name="SwiGLU", replaces=None,
        key_properties="smooth gated activation", confidence=0.7,
    )
...
def test_parameter_model():
    ...
    param = Parameter(
        id=1, name="Inference Latency", description="Time to generate output tokens",
        raw_strings=["inference time", "latency"], paper_ids=["2401.12345"],
        taxonomy_version=1, embedding=[0.1] * 768,
    )
...
def test_architecture_variant_model():
    ...
    variant = ArchitectureVariant(
        id=1, slot_id=1, name="Grouped-Query Attention",
        replaces=[2, 3],
        properties="reduces KV cache by sharing keys/values across query groups",
        paper_ids=["2305.13245"], taxonomy_version=1, embedding=[0.1] * 768,
    )
...
def test_agentic_pattern_model():
    ...
    pattern = AgenticPattern(
        id=1, name="Reflexion", category="single-agent",
        ...
        paper_ids=["2303.11366"], taxonomy_version=1, embedding=[0.1] * 768,
    )
...
def test_matrix_cell_model():
    ...
    cell = MatrixCell(
        improving_param_id=1, worsening_param_id=2, principle_id=3,
        count=5, avg_confidence=0.82,
        paper_ids=["2401.12345", "2402.67890"], taxonomy_version=1,
    )
    assert cell.count == 5
...
def test_taxonomy_version_model():
    ...
    tv = TaxonomyVersion(
        version_id=1, created_at=datetime(2026, 3, 20, 12, 0, 0),
        paper_count=200, param_count=25, principle_count=35,
    )
...
def test_ideation_gap_model():
    ...
    gap = IdeationGap(
        id=1, report_id=1, gap_type="sparse_cell",
        description="Tradeoff (accuracy, latency) has only 1 known principle",
        related_params=[1, 2], related_principles=[3], related_slots=[],
        score=0.8, llm_hypothesis=None,
        created_at=datetime(2026, 3, 20), taxonomy_version=1,
    )
...
def test_ideation_report_model():
    ...
    report = IdeationReport(
        id=1, created_at=datetime(2026, 3, 20),
        taxonomy_version=1, paper_batch_size=50, gap_count=10,
    )
...
def test_explanation_result_model():
    ...
    result = ExplanationResult(
        resolved_type="parameter", resolved_id=1, resolved_name="Inference Latency",
        narrative="Inference latency is...", evolution=["v1", "v2"],
        tradeoffs=[{"improving": 1, "worsening": 2}],
        connections=["Model Size"], paper_refs=["2401.12345"],
