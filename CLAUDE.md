...
- **Scoop-check** — `lens scoop-check` / `run_scoop_check` (`knowledge/scoop_check.py`) verifies idea-card novelty: for each card with `novelty_status='unchecked'` it queries OpenAlex (`search_openalex`, free polite pool) by the card's `signature_terms`, LLM-judges `novel|overlaps|scooped` against the retrieved abstracts, and annotates the card (`novelty_status`, `prior_art`, `novelty_note`). Idempotent and fail-soft (cards only leave `unchecked` on a real verdict); `--limit` caps checked cards without starving unreached ones; `--max-terms` (default 5) caps signature-term searches per card (1 OpenAlex request each) to fit OpenAlex's daily credit budget — the free tier is ~100 searches/day, so e.g. `--max-terms 3` keeps 33 cards under one day's budget; intentionally not run in the monitor cron.
