...
- **Idea cards** — with `ideate_with_llm`, `run_ideation_with_llm` (`monitor/ideation.py`) turns each structurally-found gap into a structured `IdeaCard` (`idea_cards` table): it selects `ideation_pattern`(s), grounds the gap in its referenced vocabulary, and generates one validated, falsifiable card (title, mechanism, falsification, differentiation, signature_terms, paper_ids) via a single LLM call parsed with `json_repair`. Structural gap-finding is unchanged; bad/`null`/failed LLM responses skip only that gap.
