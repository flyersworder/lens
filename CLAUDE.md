...
- **Scoop-check** — `lens scoop-check` / `run_scoop_check` (`knowledge/scoop_check.py`) verifies idea-card novelty: for each card with `novelty_status='unchecked'` it queries Semantic Scholar (`search_semantic_scholar`, free tier) by the card's `signature_terms`, LLM-judges `novel|overlaps|scooped` against the retrieved abstracts, and annotates the card (`novelty_status`, `prior_art`, `novelty_note`). Idempotent and fail-soft (cards only leave `unchecked` on a real verdict); intentionally not run in the monitor cron.
