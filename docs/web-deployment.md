...
| **Database (prod)** | Turso `lens-prod` (`libsql://lens-prod-flyersworder.aws-eu-west-1.turso.io`) | Live API source. As of 2026-07-19 (post-recovery): **477 papers (all 477 embedded)**, 212 vocab (all embedded), 299 matrix cells, 380 tradeoff / 502 architecture / 270 agentic extractions, 6,676 ideation gaps, **40 idea_cards** (all grounded; 1 scoop-checked so far, rest pending Monday cron), 2,224 event-log rows. Embeddings 1536-dim `text-embedding-3-small` (matches runtime query space — no truncation). Backup of the recovered corpus lives in the Turso `lens-prod-restore` branch (safe to delete once verified). |
