...
> **2026-07-19 corpus-recovery update.** Earlier the same day a manual publish + a `corpus-snapshot` bootstrap re-upload accidentally overwrote the weekly-cron-accumulated corpus (**477 papers / 299 matrix cells / 1,152 extractions**) with a stale 77-paper local DB, on both `lens-prod` and the `corpus-snapshot` release. Recovered via **Turso point-in-time restore** (`turso db create <branch> --from-db lens-prod --timestamp <before-overwrite>`, starter plan) → reverse-copied to local (`scripts/restore_from_turso.py`) → re-embedded + re-ideated (`scripts/enrich_restored_db.py`) → republished to both `lens-prod` and `corpus-snapshot`. **Lesson:** pull `corpus-snapshot` to local before any manual publish — local drifts stale between sessions. Idea cards were **regenerated fresh (40, all grounded via the new `_card_paper_ids` bridge fallback)**, replacing the earlier 33; only 1 was scoop-checked at recovery time (OpenAlex daily budget exhausted, HTTP 429) — the rest fill in via the Monday cron after the UTC-midnight reset. The monitor cron runs `lens scoop-check --max-terms 3 --limit 30` between *Run monitor* and *Publish*, re-uploading verdicts into `corpus-snapshot` so durability is self-maintaining.
...
| **Endpoints** | `/api/health`, `/api/search`, `/api/analyze`, `/api/explain`, `/api/stats`, `/api/ideas`, `/api/track`, `/api/usage-summary` | Defined in `api/index.py`, served by the `lens-api` project. `stats` aggregates corpus counts (5-min in-process cache); `ideas` returns the vetted idea cards (novel-first, `unchecked` excluded, degrades to an empty envelope — never 500s) backing the `/ideas` showcase; `track` writes a row into `usage_events` on Turso (lazy DDL, no-op when `TURSO_*` is unset). |
| **Frontend** | Next.js 16 App Router at repo root (`app/`, `lib/`) | Pages: `/` (search + landing), `/analyze`, `/explain/[concept]` (+ `/explain` landing), `/ideas` (vetted-novelty idea showcase — receipt-forward cards with `novel`/`overlaps`/`scooped` badges + prior-art receipts), `/usage` — plus a global `StatsBar`. Uses Tailwind; no shadcn registry. All pages are `"use client"` and fetch on mount; views/submissions tracked via `lib/api.ts#track()` (sendBeacon). |
| **Database (prod)** | Turso `lens-prod` (`libsql://lens-prod-flyersworder.aws-eu-west-1.turso.io`) | Live API source. As of 2026-07-19 (post-recovery): **477 papers (all 477 embedded)**, 212 vocab (all embedded), 299 matrix cells, 380 tradeoff / 502 architecture / 270 agentic extractions, 6,676 ideation gaps, **40 idea_cards** (all grounded; 1 scoop-checked so far, rest pending Monday cron), 2,224 event-log rows. Embeddings 1536-dim `text-embedding-3-small` (matches runtime query space — no truncation). The recovered corpus now lives in prod + `corpus-snapshot` + local (the transient `lens-prod-restore` PITR branch used during recovery was deleted once verified). |
